...
async def fetch_player(tag: str) -> dict | None:
    ...
    # Пробуем brawltime.ninja
    url = f"https://brawltime.ninja/api/player/{tag}"
    headers = {
            ...
            async with session.get(url, headers=headers, timeout=aiohttp.ClientTimeout(total=10)) as resp:
                if resp.status == 200:
                    ...
                    return {
                        ...
                        "highestTrophies": player.get("highestTrophies", player.get("highestTrophies", 0)),
                        "brawlers": brawlers,
                    }
    except Exception as e:
        print(f"brawltime.ninja error: {e}")

    # Запасной вариант — brawlify
    try:
        url2 = f"https://api.brawlify.com/v1/players/%23{tag}"
        async with aiohttp.ClientSession() as session:
